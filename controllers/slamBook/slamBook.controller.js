...
exports.deleteSlamBookQuestion = async (req, res) => {
  ...
  if (answered) {
    return res
      .status(400)
      .send(
        `Cannot delete the question now as it has been answered already by ${answered.length} users`,
      );
  }
...
exports.upsertAnswer = async (req, res) => {
  debug('function: upsertAnswer(), Purpose : Route to upsert a slambook answer');
