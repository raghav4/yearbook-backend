...
exports.loggedInUser = async (req, res) => {
  ...
  return res.status(200).send(
      _.pick(user,
             [ 'credentials', 'info', 'deptSection', 'socialHandles', '_id' ]),
  );
};

exports.getClassUsers = async (req, res) => {
  debug(
      'Function : getClassUsers(), Purpose : Route to get all the class users');
  const user = await User.findById(req.user._id);
...
exports.updateDetails = async (req, res) => {
  ...
  return res.status(200).send(
      _.pick(user,
             [ 'credentials', 'deptSection', 'socialHandles', 'info', 'bio' ]),
  );
};
...
exports.updateProfilePicture = async (req, res) => {
  debug(
      `function: UpdateUserProfilePicture(), Purpose: to update the profile picture, files: ${
          req.files}`,
  );
  const user = await User.findById(req.user._id);
...
exports.resetPassword = async (req, res) => {
  ...
  if (req.body.password === req.body.newPassword) {
    return res.status(400).send(
        'New Password cannot be equal to the last password');
  }
...
exports.getAllUsers = async (req, res) => {
  debug(
      'Function : getAllUsers(), Purpose : Route to get all the registered Users');
